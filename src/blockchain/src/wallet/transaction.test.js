...
describe('Transaction', () => {
    ...
    //vamos a buscar el output que emite para ver si se le ha restado lo que envia de su dinero
    it('output the amount substracted from the wallet balance', () => {
        //iteramos y va a devolvernos cada elemento de la array
        const output = transaction.outputs.find(({ address }) => address === wallet.publicKey);
        expect(output.amount).toEqual(wallet.balance - amount); 
    });

    //miramos si la address del recipiente existe y si se le ha añadido el amount
    it('output the amount added to the reci the wallet balance', () =>{
